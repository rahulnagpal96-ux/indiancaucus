...
export async function createCampaign({ subject, htmlContent, type = 'email', scheduledAt = null }) {
  return sql`
    INSERT INTO campaigns (subject, html_content, type, scheduled_at, status)
    VALUES (${subject}, ${htmlContent}, ${type}, ${scheduledAt},
...
export async function getCampaigns() {
  ...
  return sql`
    SELECT c.id, c.subject, c.type, c.status, c.recipient_count, c.sent_at, c.created_at,
      c.resend_broadcast_id, c.scheduled_at,
...
// ── Welcome emails ──
// Welcome emails are transactional Resend sends (one per new subscriber), not
// broadcasts, so they don't appear in the campaigns table. This table records
// each send and its delivery lifecycle so the dashboard can show whether the
// automated welcome email actually went out, was delivered, opened, etc.
let welcomeEmailSchemaPromise
async function ensureWelcomeEmailSchema() {
  if (!welcomeEmailSchemaPromise) {
    welcomeEmailSchemaPromise = (async () => {
      await sql`
      ...
      `
      // Webhook events look the row up by Resend's email id.
      await sql`CREATE INDEX IF NOT EXISTS welcome_emails_resend_id_idx ON welcome_emails (resend_email_id)`
    })()
  }
  await welcomeEmailSchemaPromise
}

export async function recordWelcomeEmailSend({ email, firstName, resendEmailId = null, status = 'sent', error = null }) {
  await ensureWelcomeEmailSchema()
  return sql`
    INSERT INTO welcome_emails (email, first_name, resend_email_id, status, error)
    VALUES (${email}, ${firstName || null}, ${resendEmailId}, ${status}, ${error})
    RETURNING *
  `
}

// Advance a welcome email's lifecycle from a Resend webhook event. Status only
// moves forward (sent → delivered → opened → clicked); bounced/complained
// override. Timestamps are set once and idempotent so repeated events are safe.
export async function recordWelcomeEmailEvent(resendEmailId, eventType) {
  await ensureWelcomeEmailSchema()
  if (!resendEmailId) return { rowCount: 0 }
  ...
  if (eventType === 'bounced' || eventType === 'complained') {
    return sql`UPDATE welcome_emails SET status = ${eventType} WHERE resend_email_id = ${resendEmailId}`
  }
  return { rowCount: 0 }
}
...
export async function getWelcomeEmailStats() {
  ...
  return {
    ...
    sent: parseInt(x.sent),
    delivered: parseInt(x.delivered),
    opened: parseInt(x.opened),
    clicked: parseInt(x.clicked),
    bounced: parseInt(x.bounced),
    complained: parseInt(x.complained),
    failed: parseInt(x.failed),
  }
}

export async function getWelcomeEmails(limit = 100) {
  await ensureWelcomeEmailSchema()
  return sql`
    SELECT id, email, first_name, status, error, sent_at, delivered_at, opened_at, clicked_at
    FROM welcome_emails
