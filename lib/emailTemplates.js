...
// Build the list of buttons to render. Prefers an explicit `buttons` array
// ([{ text, url }, …]); falls back to the legacy single ctaText/ctaUrl, then to
// an optional default. Invalid rows (missing text or url) are dropped.
function resolveButtons({ buttons, ctaText, ctaUrl } = {}, fallback = {}) {
  const list = (Array.isArray(buttons) ? buttons : []).filter((b) => b && b.text && b.url)
  if (list.length) return list
  const text = ctaText || fallback.text
  const url = ctaUrl || fallback.url
  return text && url ? [{ text, url }] : []
}

// Render one or more buttons, centered and stacked. Works in Outlook (table
// `align="center"` + per-row `align="center"`) and modern clients.
function buttonStack(buttons, color) {
  const valid = (buttons || []).filter((b) => b && b.text && b.url)
  if (!valid.length) return ''
  const row = (b, first) => `
          <tr>
            <td align="center" style="padding-top:${first ? '0' : '12px'};">
              <table cellpadding="0" cellspacing="0" role="presentation" align="center" style="margin:0 auto;">
                <tr>
                  <td style="background:${color};border-radius:10px;">
                    <a href="${b.url}" style="display:inline-block;padding:14px 32px;color:#ffffff;font-size:15px;font-weight:700;text-decoration:none;">${b.text}</a>
                  </td>
                </tr>
              </table>
            </td>
  ...
  return `
        <table cellpadding="0" cellspacing="0" role="presentation" width="100%" style="width:100%;">
          ${valid.map((b, i) => row(b, i === 0)).join('')}
        </table>`
}

export function buildEventEmail({ imageUrl, title, date, location, description, ctaText, ctaUrl, buttons }) {
  return wrap(`
    ${header()}
    ${imageUrl ? `
    <tr>
    ...
    <tr>
      <td style="background:#ffffff;padding:36px 40px 28px;">
        <h1 style="margin:0 0 16px;color:#1a2744;font-size:28px;font-weight:800;line-height:1.2;text-align:center;">${title}</h1>
        ...
        ${buttonStack(resolveButtons({ buttons, ctaText, ctaUrl }, { text: 'Learn More', url: BASE + '/events' }), '#e85d04')}
      </td>
    </tr>
    <tr>
...
export function buildNewsletterEmail({ imageUrl, headline, body, ctaText, ctaUrl, buttons }) {
  return wrap(`
    ${header()}
    ${imageUrl ? `
    <tr>
    ...
    <tr>
      <td style="background:#ffffff;padding:36px 40px;">
        <h1 style="margin:0 0 20px;color:#1a2744;font-size:26px;font-weight:800;line-height:1.2;text-align:center;">${headline}</h1>
        <div style="color:#374151;font-size:15px;line-height:1.8;">${(body || '').replace(/\n/g, '<br>')}</div>
        ${(() => {
          const list = resolveButtons({ buttons, ctaText, ctaUrl })
          return list.length ? `<div style="margin-top:28px;">${buttonStack(list, '#1a2744')}</div>` : ''
        })()}
      </td>
    </tr>
    ${footer()}
...
export function buildDonationEmail({ imageUrl, headline, body, ctaText, goalAmount, raised, buttons }) {
  const donationButtons = [
    { text: ctaText || 'Donate Now', url: BASE + '/donate' },
    ...(Array.isArray(buttons) ? buttons : []).filter((b) => b && b.text && b.url),
  ]
  return wrap(`
    ${header()}
    ${imageUrl ? `
    ...
    <tr>
      <td style="background:#ffffff;padding:36px 40px 28px;">
        <h1 style="margin:0 0 16px;color:#1a2744;font-size:26px;font-weight:800;line-height:1.2;text-align:center;">${headline}</h1>
        <div style="color:#374151;font-size:15px;line-height:1.8;margin-bottom:28px;">${(body || '').replace(/\n/g, '<br>')}</div>
        <div style="margin-bottom:16px;">${buttonStack(donationButtons, '#e85d04')}</div>
        <p style="margin:0;color:#9ca3af;font-size:13px;text-align:center;">Every dollar directly funds our community events. Tax-deductible 501(c)(3).</p>
      </td>
    </tr>
    <tr>
